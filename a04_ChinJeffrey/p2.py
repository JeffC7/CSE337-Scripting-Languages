...
def find_contact():
    ...
    remove_result_buttons_frame.delete(0, END)
    for c in results:
        result_field.insert(END, stringify_contact(c)) 
        remove_result_buttons_frame.insert(END, stringify_contact(c))
        # button = Button(remove_result_buttons_frame, font = 12, text = stringify_contact(c), bg = "red", width = 20, height = 5, anchor=W)
        # button.pack()
...
def change_view():
    ...
    if radio_var.get() == current_view:
        ...
        remove_result_frame.pack_forget()
        current_view = 1
        build_add_view()
...
def delete_contact():
    global value_delete
    value_arr = value_delete.split("\n")
    name_arr = value_arr[0][6:len(value_arr[0])-1]
    name_arr = name_arr.split(" ")
    first_name = name_arr[0]
    last_name = name_arr[1]
    phone = value_arr[1][7:len(value_arr[1])-1]
    email = value_arr[2][7:len(value_arr[2])-1]
    index_of_selected = 0
    for q in contact_list:
        if q["fn"] == first_name and q["ln"] == last_name and q["phn"] == phone and q["eml"] == email:            
            contact_list.pop(index_of_selected)
            break
        index_of_selected+=1
    find_contact()
    messagebox.showinfo("Remove Successful", first_name + last_name + " has been removed from the contact list.")


remove_result_frame = Frame(root, bg = "cyan")
remove_result_lbl = Label(remove_result_frame, font = my_font, width = 16, anchor = W, text = "Results:", bg = "cyan")
remove_result_buttons_frame = Listbox(remove_result_frame, bg = "white", width = 80, height = 25)
delete_button = Button(remove_result_frame, font = my_font, text = "DELETE", bg = "red", command = delete_contact)

def onselect(evt):
    w = evt.widget
    index = int(w.curselection()[0])
    global value_delete
    value_delete = w.get(index)

remove_result_buttons_frame.bind('<<ListboxSelect>>', onselect)
...
def build_remove_view():
    ...
    remove_result_buttons_frame.pack(side=LEFT)
build_add_view()
